    ...
    <div className="space-y-8">
      <Card>
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              <h3 className="font-semibold text-sm text-muted-foreground">Class</h3>
              <p className="font-medium">{assessmentData.class}</p>
            </div>
            <div>
              <h3 className="font-semibold text-sm text-muted-foreground">Duration</h3>
              <p className="font-medium">
                {formatDate(assessmentData.startDate)} - {formatDate(assessmentData.endDate)}
              </p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-6">
            <div className="border rounded-lg p-3">
              <h3 className="font-semibold text-sm text-muted-foreground">Max Score</h3>
            ...
            <div className="border rounded-lg p-3">
              ...
              <p className="font-medium text-lg">{assessmentData.passingScore}</p>
            </div>
            <div className="border rounded-lg p-3">
              <h3 className="font-semibold text-sm text-muted-foreground">Students Attempted</h3>
              <p className="font-medium text-lg">{assessmentData.studentsAttempted}</p>
            </div>
            <div className="border rounded-lg p-3">
              <h3 className="font-semibold text-sm text-muted-foreground">Students Passed</h3>
              <p className="font-medium text-lg">{assessmentData.studentsPassed}</p>
            </div>
          </div>
        </CardContent>
      </Card>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Review all questions in this assessment</CardDescription>
        </CardHeader>
        <CardContent>
          {assessmentData.questions.length === 0 ? (
            <div className="text-center p-4 border border-dashed rounded-lg">
              <p className="text-muted-foreground">No questions have been added to this assessment.</p>
            </div>
          ) : (
              ...
              {assessmentData.questions.map((question, index) => (
                <div key={question.id} className="border rounded-lg p-4">
                  ...
                  {question.answerType === "Image" && question.imageUrl && (
                    <div className="mt-4">
                      <h4 className="text-sm font-medium">Image Answer Required:</h4>
